...
int main(int argc, char **argv) {
  ...
  // We use a pre-generated ring buffer of random IPs to save memory and startup time.
  const size_t ring_buffer_size = std::min<size_t>(options.networks, 65536);
  std::vector<std::string> networks_ring;
  networks_ring.reserve(ring_buffer_size);
  for (size_t i = 0; i < ring_buffer_size; ++i) networks_ring.push_back(network_text(i));
  ...
  const auto insert_start = std::chrono::steady_clock::now();
  {
    ...
    for (unsigned t = 0; t < options.threads; ++t) {
      workers.emplace_back([&, t] {
        ...
        for (size_t i = t; i < options.networks; i += options.threads) {
          const data_id_t record = records[i % records.size()];
          if (!options.batch) {
            if (!builder.insert_network(networks_ring[i % ring_buffer_size], record)) rejected++;
            continue;
          }
          NetworkEntry entry;
          if (!builder.parse_network(networks_ring[i % ring_buffer_size], record, entry)) {
            rejected++;
            continue;
          }
