...
inline void run_worker_thread(
  ...
  for (int i = 0; i < num_conns; ++i) {
    ...
    std::array<uint8_t, 16> rand_ip{};

    if (!fixed_ips.empty()) {
      const auto &fip = fixed_ips[(c.sequence + c.conn_index) % fixed_ips.size()];
      is_v6 = fip.is_ipv6;
      ip_ptr = fip.bytes.data();
    } else {
      is_v6 = (stats.rng.next_u32() & 1) != 0;
      if (is_v6) {
        uint64_t h = stats.rng.next_u64();
        uint64_t l = stats.rng.next_u64();
        std::memcpy(rand_ip.data(), &h, 8);
        std::memcpy(rand_ip.data() + 8, &l, 8);
      } else {
        uint32_t v4 = stats.rng.next_u32();
        std::memcpy(rand_ip.data(), &v4, 4);
      }
      ip_ptr = rand_ip.data();
    }
      ...
      while (true) {
        ...
        if (c->rx_received == c->expected_frame_size) {
          ...
          if (now_tick < phase_end) {
            ...
            if (!fixed_ips.empty()) {
              const auto &fip =
                  fixed_ips[(c->sequence + c->conn_index) % fixed_ips.size()];
              is_v6 = fip.is_ipv6;
              ip_ptr = fip.bytes.data();
            } else {
              is_v6 = (stats.rng.next_u32() & 1) != 0;
